...
| pane-terminal lease | the exclusive claim one live interactive operation holds on one terminal pane. Claims in different panes do not contend; two claims in one pane do. It is minted and validated by the host's terminal owner and grants no permission over an Agent session |
...
The host owns one non-contextual terminal owner, built and delivered
...
provider factory closes over the delivered owner and must present that same
...
ownership, lifecycle, settlement, and replay do not depend on tmux behavior.
