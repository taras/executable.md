...
separator. The untagged version-1 record holds one opaque string instead, and is
reconciled rather than refused: the build that wrote it spelled an identity
`origin#key@revision`, so a run resuming under one compares its own structural
identity under exactly that mapping, and only for a capability — the
component-answer arm did not exist then, so no version-1 string ever described
one. The run's own ceiling table is keyed by those terms encoded rather than
joined, because a joined key is only as unique as the separator is illegal and
nothing makes a space illegal in an origin or a component key: two entries whose
terms differ only in where a space falls each keep their own admitted requests.
Only the tables the selection reached take part, so changing a write table a
read-only admission never drew on changes nothing it was granted under. Deciding
