...
| provider-native session identity | the identity a proven provider adapter admits for one provider-owned durable session, distinct from the logical XMD session key and ACP/ACPX record identity. Provenance is `provider-returned` when the provider explicitly returns the identity it created, or `client-allocated` when the adapter allocates it under a declared schema and supplies it through a provider-supported creation input. XMD never admits an authored value, ACP/ACPX identity, UUID-looking string or CLI observation by inference |
| executable build binding | the filtered provider-owned compatibility identity that constrains which native and ACP processes may accept one client-allocated session's history: for Claude V1, a canonical provider-reported version plus SHA-256 digest of the canonical executable target. It is retained in the construction route and prepared launch record, and is distinct from the invocation-scoped live executable path, raw environment and argv, which are never durable |
| construction route | the durable, create-once record of how one logical agent session was constructed — `acp-first` or `client-native` — keyed by provider, resolved agent and resolved session key. It is claimed atomically before anything is established on that session, decides which identity and executable build a later attachment must use, and never converts |
| session lease | the host-supplied, kernel-released exclusive claim on one logical agent session's live ownership, keyed by a digest of its natural key. It answers acquired, busy, or unavailable, is never waited on, and is held until the work it protects can no longer act. A provider-local flag, queue, pid file, heartbeat or stale-file timeout is not one, and a host that installs none refuses client-native work rather than acting without an answer |
...
| native launcher | the host-owned seam that reserves that lease, flushes what the document has produced, spawns the exact invocation-scoped live executable selected by the provider adapter with the terminal inherited, and reports its terminal status and nothing else. It is not `exec`, whose children are piped, captured and journaled |
...
The same rule shapes agent sessions, where the two things that must not be
split are choosing a construction route and creating the first provider-side
session under it. Reading a store and then establishing a session are two steps,
and a sibling provider sharing that store can create and prompt the session in
between — so the route is published atomically, before anything exists, and the
publication that wins is the one every other caller adopts or refuses against.
Route reconciliation and the first provider-side creation happen while one
caller holds the session authority: the durable route, and for client-native
work the host-supplied lease.
...
An Agent-session mapping is the provider-owned durable home of the logical
session's admitted native identity, provenance, instruction digest, launcher,
and executable build binding. A client-allocated launch reconciles that mapping
with its prepared journal record by natural key; exact accounts agree, absence
creates once, and conflict refuses without replacement. Host executable paths
remain live provider capabilities outside both the database and journal. A
later process resolves a path again and proves its filtered build equals the
retained binding before native or ACP history acceptance.
...
Git-host mutations — Git push, pull-request upsert and issue creation — share
...
| `xmd test --target` | selects one section of a single test document, composing the existing document-target catalog, selector and projection with the testing host: the positional value stays a literal path whose `#` and `%` are filename characters, the option value alone is a selector, it must resolve to exactly one addressable target, sibling subtrees never execute, and a directory input refuses before any discovered document runs | built on the #519 stack |
...
| native session launch (`<Session.Launch>` / `Agent.launch()`) | prepares one durable coding-agent session from the rendered body of `<Session.Launch>` and hands the provider's native UI the terminal for that exact session, then continues after it exits. Identity is either explicitly provider-returned or client-allocated through a proven adapter; Claude's native-first branch allocates one UUID, publishes its client-native construction route, installs instructions through a restrictive private file, creates with `--session-id`, resumes with `--resume`, and later reattaches ACP with the same `resumeSessionId`. A client-allocated history is accepted only through its retained executable version and digest: native launch receives the matching invocation-local path, ACP receives it only in the bound adapter child's environment, and `executable-binding-refused` stops mismatches before detach, spawn, ensure or history acceptance. `agent_session_launch` retains `prepared`, `detached`, and `exited`; completed replay observes no executable and incomplete replay revalidates the binding and preserves identity | provider-neutral path built on PR #518; client allocation and executable binding built on the #519 stack; installed-Claude proof and advertisement unbuilt on #520; `Agent.AddDir` remains unbuilt |
| agent session coordinator | the provider-owned authority for one natural key of provider, resolved agent and logical session: it publishes the durable construction route before anything is established, and takes the host-supplied session lease for live client-native work. Provider scopes that can observe the same session records coordinate through the same authority; stores whose namespaces cannot identify one are a configuration refusal rather than a reason to fall back to provider-local queues. Route selection is create-once durable state, while live ownership is a contextual kernel-released lease in the same namespace — the Deno source entrypoint and the compiled binary install one, and Node and Bun install none and therefore refuse client-native launch and attachment. Provider-local contextual state may optimize routing but authorizes no deletion, identity substitution or concurrent ownership | built on the #519 stack |
