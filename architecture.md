...
| Issue tracker | what a lexical Issue context names: a credential-free URL for the container new issues are created in, together with an optional explicit provider discriminator. It is replaceable composition data; it requests a destination and grants nothing |
...
| provider discriminator | the stable lower-case name of an Issue provider. It selects which installed provider may accept a request, and is resolved from the canonical target URL when the context does not state one |
...
that target is authorized is the trusted host's. The nearest Issue context
supplies a canonical target URL and an optional provider discriminator; the
host's private ceiling decides whether that target may be reached at all.
Context and routing middleware select and narrow within the ceiling and can
never widen it, which is what keeps a replaceable context from being a grant.
...
| `<Issue>` | creates one issue in the container the nearest lexical Issue context names, through a boundary of its own rather than the Git host's: exactly `title`, `description`, optional `tags` and optional `assignee`, no repository/url/provider/token/label/milestone/project/comment/close or approval prop, it renders nothing, and `as` binds exactly `{ url }`. A paired invocation is refused on the shape of the invocation, before the tracker is read and before routing. The context carries a credential-free `url` and an optional `provider`; the URL is canonicalized and the provider resolved from a table of well-known hosts the trusted host installs beside its adapters — `github.com` to `github`, `*.atlassian.net` to `atlassian` — so the shared boundary carries the mechanism and never a provider's name, unless the context states one, which wins and never falls back on refusal. A nested context replaces the whole target for its descendants and members are never merged. The context is composition data, not authority: the trusted host installs an adapter-private target ceiling beside its credentials, and a target outside it fails before external observation. Routing is one request-only contextual operation, `executablemd.workflow.issue`, under #297's one-surface authority rule. Identity is engine-derived; the natural key is `{ canonicalTargetUrl, runId, expansionId }` within the resolved provider's namespace and the fingerprint covers provider, canonical URL, title, description, normalized tags and assignee. Tags are deduplicated and code-point sorted because order is not identity, and title is never identity. Reconciliation is journaled as `issue_effect`, retaining the normalized request, provider name, pre-state, observations, decision and normalized result — no credential, payload, endpoint or host path. Deferral classification, approval and PullRequest provenance are workflow policy expressed in document structure or Issue middleware, not members of this primitive | built on the #296 stack; GitHub adapter, Deno host |
