...
| `<Issue>` | records one approved deferred obligation as a Git-host issue in the same Repository as the retained PullRequest evidence it names: required `finding`, `disposition`, `pullRequest`, `title`, `rationale`, `dependencyImpact` and `intendedTiming`, rendered content as evidence, no repository/provider/token/label/assignee/milestone/project/comment/close controls, and no implicit pull-request behavior. Only `defer` reaches a durable suspension request whose body is the exact normalized Issue request; `{ approved: false }`, `rejected`, `fix-now` and `inserted-repair` create no issue and observe no provider, while `{ approved: true }` authorizes only that request. A successful `PullRequest` result from the same run is admitted before Git-host observation. Reconciliation keys the Repository identity, PullRequest provider identity and finding ID; it creates once, adopts or updates one compatible open issue, and refuses closed, multiple or conflicting matches rather than overwriting. Its `as` value is stable issue evidence — filtered Repository identity, PullRequest number and URL, provider issue identity, issue number, URL, state `open`, finding ID and decision — not a live issue snapshot; the first adapter works over `github.com`, writes one origin marker in the issue body, uses `GH_TOKEN` then `GITHUB_TOKEN`, and confines endpoint, credential and raw payloads to the per-invocation provider closure | built on the #296 stack, Deno provider only |
