...
| transactional Git effects (`Git.Switch` / `Git.Add` / `Git.Commit`) | publish local Git mutations with their journal result; the enclosing Repository and the contextual working directory select which retained checkout one runs in, and neither observation carries authority over it | `Git.Switch` built on the #294 stack, Deno provider only; `Git.Add` and `Git.Commit` defined in `specs/workflow-workspace-spec.md`, unbuilt (#294) |
