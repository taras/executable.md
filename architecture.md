...
| workflow component bundle | a workflow root declares a closed set of authored Markdown components; `start` and `resume` read them from the definition's own pinned commit, the V2 descriptor retains name, canonical path and blob object ID per component, and canonical core resolves those names and holds both live import and retained history to that exact bundle | built on the #301 stack; the full adversarial implementation loop, its scheduling, and generated-XMD admission remain unbuilt (#300, #369) |
