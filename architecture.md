...
| `<Verbose>…</Verbose>` | renders its content only when the CLI host's already-resolved verbose flag is true, and otherwise renders the empty string without expanding the content. It is a run-profile registered default, not core syntax and not reserved, so a repository `Verbose.md` is chosen ahead of it. The flag is closed over by the host-installed function rather than read from document-reachable context, props, bindings or middleware. It owns no resource, authority or durable effect of its own; replay follows ordinary expansion under the host options for that execution | built on this stack |
