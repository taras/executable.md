...
| Issue tracker | what a lexical `<IssueTracker>` names: a credential-free URL for the container new issues are created in, together with an optional explicit provider discriminator. It is replaceable composition data; it requests a destination and grants nothing |
...
| `<Issue>` | creates one issue in the container the nearest lexical `<IssueTracker>` names, through a boundary of its own rather than the Git host's: exactly `title`, `description`, optional `tags` and optional `assignee`, no repository/url/provider/token/label/milestone/project/comment/close or approval prop, it renders nothing, and `as` binds exactly `{ url }`. A paired invocation is refused on the shape of the invocation, before the tracker is read. The tracker carries a credential-free `url` and an optional `provider`; the URL is canonicalized — a credential, a query and a fragment are refused rather than stripped — and a nested tracker replaces the whole value for its descendants, never merging members. It is composition data, not authority: the provider holds an adapter-private ceiling beside its credentials, and a target outside it fails before anything is sent. Upsert is one stable contextual operation, `executablemd.workflow.issue`, with the single member `upsert(issue, options)`; a provider is ordinary middleware around it, matching its own URLs without a discriminator and only its own name with one. Once middleware matches it owns the answer — it never delegates afterwards, and nothing catches its refusal to try somebody else — and a destination everyone delegated reaches `NoIssueProvider` unchanged. `issue_effect` derives the idempotency key from the canonical target and the run's own effect identity, durably wraps the one call, and retains the request and the URL — no credential, payload, endpoint, provider identity or host path. Observing, adopting, creating once and recovering an interrupted creation are the provider's, because they are knowledge about what a service can prove; title is never identity, and tags are a code-point-sorted set | built on the #296 stack; GitHub middleware, Deno host |
