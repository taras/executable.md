...
The durable record follows the ordinary rules. Each occurrence claims the
identity this execution minted, performs one `syntax_catalog` observation, and
retains exactly `{ catalog: string }`. A continuation hostile-parses that record
