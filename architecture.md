...
The package is split so that arranging a grid costs nothing. Its root entry
point is provider-neutral data — the layout a column count and a pane order
derive — and imports no engine. The `/xmd` subpath is the executable-Markdown
integration, and it is the only side that depends on core. Core depends on
neither, so the dependency runs core ← `@executablemd/terminal/xmd` ← the CLI
with no cycle.
...
The grid and its panes are **installed structural syntax**, contributed to an
execution by `@executablemd/terminal/xmd` rather than owned by the engine. One
`ExecutionInstallation` declares both forms and supplies the implementation that
expands them, so a profile that installs the package has the syntax, its
validation, its inspection entry and its expansion together, and a profile that
installs none has no grid syntax at all. Canonical core contains no name, rule
or dispatch branch for either form.

What the declarations say is the contract. `<Terminal.Grid>` is paired, requires
a positive integer `columns`, and contains at least one direct
...
A structural form is necessary here, and an ordinary component cannot be one. A
function component receives its content as one rendered string, after the
effects in that content have already run; it cannot inspect direct authored
children or begin them concurrently. The installed parent instead receives its
accepted children as regions it expands explicitly, and core validates the
complete static grid before any provider contact,
...
## The declaration catalog

`ExecutionInstallation.declarations` is the one declarative input for the syntax
a trusted host adds to an execution. Its entries form a discriminated catalog
with two arms.

A `markdown` entry is first-party Markdown the host ships and names — the tier
this section describes. A `structural` entry is an installed parent or child
form: syntax the engine does not own, whose parent decides how its own direct
children are partitioned and expanded. The installation that declares a
structural form supplies the `expand` that implements it, and declaring one
without the other refuses the execution before authored content runs. So one
installation is the atomic owner of the syntax it contributes and of the
implementation behind it, and a profile cannot describe a form differently from
the form execution selects.

Core copies and validates the whole catalog before any installation hook,
middleware or document code runs. Inspection and document validation consume
that same captured data — they never invoke `install()` or `expand()` — which is
what stops `xmd syntax`, `xmd plan` and a run from describing different
environments. A name two arms claim, a name a reserved registration or a
workflow bundle already claims, a structural child naming a parent its own
installation does not declare, and a parent with no child form are each refused
where the catalog is installed rather than where a document happens to write the
name.
...
| --- | --- | --- |
| `xmd syntax` | describes every structural construct and every selected component the production `run` profile would let a document write in the contextual working directory, as deterministic Markdown or as version-3 JSON, from one construction. `xmd syntax Elicit` names one component instead and renders its symbol metadata followed by the long-form documentation the owning package ships, through the same selection, index and renderer `<Syntax names={…}>` uses. The structural forms it lists come from the exact installation factory an ordinary run installs from, not from a table written beside it. Inspection only: it registers the run profile's declarations in a bounded scope, reads their declaration data without invoking `install()` or `expand()`, and reads the filesystem for which files exist and, for a selected Markdown component, that file's frontmatter. It runs no body, imports no repository TypeScript module, installs no provider, mints no authority and writes no journal. An include it cannot enumerate — a selection-relevant symbolic link to a directory beneath it included — fails the whole request rather than printing a healthy subset. The Markdown renderer is core's rather than this command's, because canonical `<Syntax />` prints the same symbols for a running document and the two must be the same bytes for the same site | built on the #632 stack, with the shared Markdown renderer added on this stack |
...
| installed structural syntax | a trusted host adds a coordinated parent and child form to one execution through `ExecutionInstallation.declarations`, and the same installation supplies the `expand` that implements them. One discriminated catalog carries both those entries and the declared Markdown tier; core copies and validates it before any installation hook, middleware or document code runs, and selection, `xmd syntax`, `xmd plan`, document validation and execution all read that one captured catalog. A declaration states its name, origin, accepted forms, closed props schema, documentation, placement and its own diagnostic wording; it carries no implementation, scope, raw AST, capture, return, private component, durable identity or provider. For one occurrence core decides the authored form and the direct-child placement from source, evaluates and schema-validates the parent's props and then each accepted child's in source order, detaches and deeply freezes the result, publishes one authentic request to public `Execution.expand` middleware — which may observe, refuse by throwing, or delegate, and which receives no regions and no implementation — and then hands that request and the source-ordered child regions to the captured owner alone. A forged, changed, sibling, cross-execution or twice-delegated request is a protocol refusal, and an implementation failure becomes the parent's positioned checked failure, carrying a JSON `Error.cause` as evidence. Each region expands explicitly to a resource-scoped, per-chunk rendezvous stream of `{ text, exact }`: the producer does not advance past a delivered chunk until the consumer advances, region output never travels through `DocumentOutput`, no default writer copies an unconsumed region anywhere, and leaving the handler's scope halts and joins the producer | built on the #806 stack |
| terminal grid (`<Terminal.Grid>` / `<Terminal>`) | replaces the root foreground terminal with one provider-neutral composite whose statically declared direct panes begin concurrently, stay independently interactive, preserve their final statuses until the reader closes the composite, and tear down completely before document execution continues. A paired pane expands isolated document flow; a self-closing pane runs the host's default shell. The grid owns one foreground-terminal lease, each pane owns a separate pane-terminal lease, and a pane-scoped native launcher lets `<Session.Launch>` use that pane without weakening the independent Agent session coordinator. Core validates the complete row-major layout before provider contact, attaches only after every pane is ready, contains post-attach pane failures until close, and records the ordered provider-neutral outcomes. Completed replay contacts no terminal or Agent provider; partial replay rebuilds a fresh composite, restores completed panes as statuses, and continues incomplete pane effects under their existing durable identities. Provider commands, sockets, process topology and layout identifiers remain live-only inside the provider closure. The syntax, its validation, its inspection entry and its expansion are installed by `@executablemd/terminal/xmd` rather than owned by core; on this stack that installation derives the row-major layout and refuses for want of a provider, before any pane expands or any shell starts | defined for #717; the installed syntax boundary and the providerless layout refusal are built on the #806 stack; #726 proves the persistent tmux pane-worker topology and its observable teardown boundary on macOS; first production provider is tmux in the Deno and compiled foreground hosts; controlled non-tmux provider proves the core contract; implementation unbuilt |
