...
| construction route | how one logical agent session was first constructed — ACP-first or client-native — as strict create-once durable state beside the coordinator's own records. It answers a different question from ownership and grants no right to ensure, prompt, detach, spawn, resume or accept history. It never converts, and a conflict is a refusal rather than something to reconcile |
| executable build binding | the canonical provider version and the SHA-256 of the executable's bytes, retained so a session whose creating build cannot be reproduced is refused rather than resumed. Deliberately not a path: a path stops being true when a build moves, and names host layout besides |
| executable observer | the host-built capability that resolves, canonicalizes, validates and hashes one executable and asks that exact file its version. Directly injected like the coordinator, because a resolver document middleware could replace could point the observation at one binary while the run spawns another |
| identity provenance | who chose a provider-native session identity: the provider returned it, or the client allocated it before the provider existed. Retained rather than inferred, because after the fact the two are the same string — and a client-allocated identity a reader cannot recognize is one a replay could silently replace |
| vendored runtime snapshot | third-party source carried in-package rather than resolved, because the build runs `--node-modules-dir=none --cached-only` where a package override cannot resolve. Each snapshot names the issue that carries it and the issue that removes it, and a verifier proves offline that only the declared differences exist |
...
| vendored ACPX runtime (`packages/acp/vendor/acpx`) | the ACP runtime `@executablemd/acp` executes: a source snapshot of `acpx@0.12.0` with one behavioral patch threading a transient agent-process environment to each spawned child, so a launch can bind one exact provider build into both owners of a session. Carried in-package because the release compile resolves no package overrides, verified offline against pristine upstream bytes, and reached through exactly one relative module | carried by #519; removed by #526 once a released upstream package set supplies that input and passes its removal gate |
| native session launch (`<Session.Launch>` / `Agent.launch()`) | prepares one durable coding-agent session from the rendered body of `<Session.Launch>` and hands the provider's native UI the terminal for that exact session, then continues the document after it exits. The body renders completely first and only what it rendered crosses as the instruction layer; the launch performs no model turn; the run's one foreground-terminal lease is taken before an agent is resolved, so a host with no terminal refuses without probing for an installed CLI. A session is constructed once, by one of two mechanisms, and its create-once construction route says which. On the ACP-first route the ACPX provider creates the session, installs the layer at creation, and the provider-returned identity is retained as such. On the client-native route the adapter allocates the identity inside ownership before any provider exists, the native process creates the session under it from a private instruction file, and ACP only ever attaches by name — an attachment that reports a different session is refused before a turn. A client-native session is bound to one executable build, observed by a host-built observer as the canonical version and the SHA-256 of the bytes it will spawn; ACP runtimes are partitioned by that binding, the same build is threaded to the adapter child through a transient environment the vendored ACPX snapshot carries, and a build that drifted under the same command refuses rather than resuming. Neither route ever converts into the other, and neither account is rewritten to make a later caller's true. Phases are retained as `agent_session_launch` records under one expansion identity — `prepared` before ownership is released, then `detached`, then `exited` — so a completed replay launches nothing and an interrupted one reconciles its journal against the durable route before its first live effect, resuming the retained identity rather than creating a replacement session. The public route carries an opaque one-use launch request and answers nothing; authority to run and retain a phase is delivered to the installed provider directly, so neither a returned completion nor a rebuilt request authors a launch. Every operation that can act on an advertised session takes exclusive ownership under one natural key first, through a coordinator the host built and passed in; contention refuses instead of queueing, and an owner that never proved it stopped leaves a recovery tombstone. A host that installs no way to take ownership, to say how a session was constructed, or to observe a build refuses every advertised operation before any provider work at all — before an agent is probed and before the ACP session store is read. No launch path discards persistent provider state | built on the #517 stack, extended by the #519 stack; Deno and the compiled binary assemble the host, Node and Bun refuse, no real adapter is advertised until #520's gate passes, `Agent.AddDir` is unbuilt, and all of it fails closed |
