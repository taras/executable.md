...
function echoComponent(name: string, slot?: string): FunctionComponentDefinition {
  return {
    kind: "function",
    name,
    path: `${name}.ts`,
...
interface RunOptions {
  ...
  functions?: Record<string, { slot?: string }>;
    ...
    yield* Component.around(
      {
        ...
        *importComponent([name], _next) {
          if (name in functions) {
            return echoComponent(name, functions[name].slot);
          }
          if (name in markdown) {
...
describe("capture error propagation", () => {
  ...
  it("CE11: a captured function component preserves named-slot content errors", function* () {
    const values: Record<string, unknown> = {};
    const result = yield* run(`<Card as="cap"><Missing slot="header" /></Card>AFTER`, {
      functions: { Card: { slot: "header" } },
      values,
    });
