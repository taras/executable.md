...
describe("Tier P — Eval binding interpolation", () => {
  ...
  it("P1: bare binding resolves from env.values", function* () {
    const result = interpolateEvalBindings("./server --port {port}", { port: 49821 });
  ...
  it("P2: bare binding with no env entry left verbatim", function* () {
    const result = interpolateEvalBindings("./server --port {port}", {});
  ...
  it("P3: namespaced refs not affected by eval binding pass", function* () {
    const result = interpolateEvalBindings("{meta.title} and {props.name} and {port}", {
      port: 8080,
    });
  ...
  it("P4: multiple bindings in one content", function* () {
    const result = interpolateEvalBindings("{host}:{port}", { host: "127.0.0.1", port: 3000 });
  ...
  it("P5: non-string binding converted via String()", function* () {
    const result = interpolateEvalBindings("port={port} active={active}", {
      port: 49821,
      active: true,
    });
  ...
  it("P6: complex patterns with dots are not replaced", function* () {
    const result = interpolateEvalBindings("{a.b} {c.d.e} {simple}", { simple: "yes" });
  ...
  it("P8: content with no binding syntax passes through", function* () {
    const result = interpolateEvalBindings("echo hello world", { port: 8080 });
    expect(result).toBe("echo hello world");
  });

  // P9: Underscore and dollar sign in binding names
  it("P9: underscore and dollar sign binding names", function* () {
    const result = interpolateEvalBindings("{_private} {$special} {__dunder__}", {
      _private: "a",
      $special: "b",
      __dunder__: "c",
    });
  ...
  it("P10: undefined binding value converted via String()", function* () {
    const result = interpolateEvalBindings("{x}", { x: undefined });
    expect(result).toBe("undefined");
  });

  // P11: Binding value is null
  it("P11: null binding value converted via String()", function* () {
    const result = interpolateEvalBindings("{x}", { x: null });
  ...
  it("P12: regression — template literal ${name} would be mangled", function* () {
    ...
    const result = interpolateEvalBindings("const url = `http://127.0.0.1:${port}/health`;", {
      port: 49821,
    });
  ...
  it("P13: dotted path resolves nested property", function* () {
    const result = interpolateEvalBindings("PR #{pr.meta.number}: {pr.meta.title}", {
  ...
  it("P14: deep dotted path resolves", function* () {
    const result = interpolateEvalBindings("{a.b.c.d}", { a: { b: { c: { d: "deep" } } } });
  ...
  it("P15: missing intermediate in dotted path left verbatim", function* () {
    const result = interpolateEvalBindings("{pr.nonexistent.field}", {
      pr: { meta: { title: "test" } },
    });
  ...
  it("P16: bare identifier still works with dotted path support", function* () {
    const result = interpolateEvalBindings("port={port}", { port: 8080 });
  ...
  it("P17: root key not in bindings left verbatim", function* () {
    const result = interpolateEvalBindings("{unknown.path}", { other: "value" });
  ...
  it("P18: null intermediate in dotted path left verbatim", function* () {
    const result = interpolateEvalBindings("{pr.meta.title}", { pr: { meta: null } });
    expect(result).toBe("{pr.meta.title}");
  });

  // P19: Mixed bare and dotted in same text
  it("P19: mixed bare and dotted in same text", function* () {
    const result = interpolateEvalBindings("{pr.stats.totalFiles} files, port {port}", {
      pr: { stats: { totalFiles: 5 } },
      port: 3000,
    });
