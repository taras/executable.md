...
export interface NativeBinding {
   ...
   *
   * The adapter's, because the identity is that provider's dialect: what shape
   * it takes and what the provider will accept is knowledge about the provider,
   * not about launching in general. The provider decides whether a freshly
   * allocated candidate wins the coordinator's ownership; it does not decide
   * what one looks like.
   */
  allocate(): string;
