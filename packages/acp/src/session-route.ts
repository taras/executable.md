 ...
 * process fact, or provider history — there is nothing to say about a session
 * ACP created beyond that ACP created it. A `client-native` record says which
 * conversation the native process was told to make and under which instruction
 * layer, and nothing else: no executable path, version, digest, adapter
 * command, raw environment, argv, instruction text, credential, transcript,
 * process handle, or temporary path.
 */
    ...
    *read(key) {
    ...
    publish(candidate) {
      ...
      return scoped(function* (): Operation<AgentSessionRoute> {
        ...
        // Named, then guaranteed removable, then written. Registering cleanup
        // after the write would leave a partial one behind when the write is
        // what failed — and a staging file is state in a namespace whose whole
        // contract is that it holds exactly the winners.
