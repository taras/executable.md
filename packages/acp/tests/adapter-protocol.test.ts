...
describe("Tier EA — the embedded adapters' prompt-response metadata", () => {
  ...
  it("EA10: reopening a Codex session reports the thread the App Server reopened", function* () {
    ...
    const resuming = yield* effectionSpawn(() =>
      adapter.request("session/resume", {
        sessionId: "thread-resumed",
        cwd: process.cwd(),
        mcpServers: [],
      }),
    );
    const loading = yield* effectionSpawn(() =>
      adapter.request("session/load", {
        sessionId: "thread-loaded",
        cwd: process.cwd(),
        mcpServers: [],
      }),
    );
    const [resumed, loaded] = yield* all([resuming, loading]);
    ...
    // first.
    expect(metaOf(resumed, "agentSessionId")).toBe("canonical:thread-resumed");
    expect(metaOf(loaded, "agentSessionId")).toBe("canonical:thread-loaded");
