...
function runChild(
  ...
  return (function* (): Operation<ChildOutcome> {
    ...
    let stdout = "";
    let stderr = "";
    // Every registration is named and removed by this operation's own teardown:
    // a cancelled wait leaves nothing attached to a child it no longer owns.
...
function ptyRun<T>(
  ...
  return scoped(function* (): Operation<T> {
    ...
    // Named and removed by this operation's own teardown, so a cancelled proof
    // leaves nothing attached to the pty it no longer owns.
    const onDisplay = (chunk: Buffer): void => {
    ...
    const onDiagnostic = (chunk: Buffer): void => {
      channels.diagnostic(chunk);
    };
    const onFailure = (error: Error): void => failed.reject(error);
    ...
    const screenChild = child;
    const screenOut = screenChild.stdout;
    const screenErr = screenChild.stderr;
    yield* ensure(() => {
      screenOut?.off("data", onDisplay);
      screenErr?.off("data", onDiagnostic);
      screenChild.off("error", onFailure);
      screenChild.off("close", onExit);
    });
    screenOut?.on("data", onDisplay);
    screenErr?.on("data", onDiagnostic);
    screenChild.on("error", onFailure);
    screenChild.on("close", onExit);
...
function* unmaterializedIsRefused(
  ...
  yield* scoped(function* () {
    ...
    const speakingOut = speaking.stdout;
    // Named and removed by this operation's own teardown, like every other
    // registration here: a cancelled probe leaves nothing on a child it has
    // stopped owning.
    const onAdapterLine = (chunk: Buffer): void => {
    ...
    yield* ensure(() => {
      speakingOut?.off("data", onAdapterLine);
    });
    speakingOut?.on("data", onAdapterLine);
