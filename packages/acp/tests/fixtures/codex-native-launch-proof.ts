...
function* settledWithin(pid: number, ms: number): Operation<void> {
  const started = Date.now();
  while (isReachable(pid) && Date.now() - started < ms) {
    yield* sleep(100);
  }
}
...
function runChild(
  ...
  return scoped(function* (): Operation<ChildOutcome> {
      ...
      try {
        if (!didClose) {
          if (child.pid) {
            ...
            yield* settledWithin(child.pid, TEARDOWN_GRACE_MS);
            if (isReachable(child.pid)) {
              killed(child.pid);
            }
          }
...
function* settleAccounts(journey: Journey, verdict: JourneyVerdict): Operation<void> {
  ...
  if (sessionKey !== undefined && sessionKey.length > 0) {
    ...
    if (route !== undefined && (yield* exists(route))) {
      const parsed: unknown = JSON.parse(yield* readTextFile(route));
      if (isRecord(parsed)) {
...
function* runNativeLaunch(
  ...
  verdict.acpDocumentCarriesMarker = (yield* readTextFile(ACP_DOCUMENT)).includes(marker);
...
function* unmaterializedIsRefused(
  ...
  yield* scoped(function* () {
    ...
    const onClose = (): void => {
      ...
      closed.resolve();
      if (pending.size > 0) {
        failed.reject(new Error("the adapter closed before answering"));
      }
    };
    yield* ensure(function* () {
      if (!child) {
    ...
    function* request(method: string, params: unknown): Operation<Record<string, unknown>> {
      const id = next++;
      ...
      try {
        ...
        return yield* race([answered.operation, failed.operation]);
      } finally {
        pending.delete(id);
      }
    }
    ...
    const created = yield* bounded("adapter-session-new", ADAPTER_MS, () =>
      request("session/new", { cwd: root, mcpServers: [] }),
    );
