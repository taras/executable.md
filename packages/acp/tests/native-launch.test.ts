...
function traceCoordinator(trace: Trace): AgentLaunchCoordinator {
  return {
    ...
    *perform(_request, phases) {
      ...
      // reaches the handoff, exactly as the real coordinator decides it: the
...
describe("Tier NL — native session launch", () => {
  ...
  it("NL18b: admission is the product's shape, not the release it was proven at", function* () {
    const codex = nativeAdapterFor("codex");
    if (codex === undefined || !bindsBuild(codex)) {
      throw new Error("codex must bind a build");
    }

    // The proof ran at 0.153.2, and that tuple is what the advertisement stands
    // on — but what the observer admits is the product line. A release this
    // build has never seen passes here, because refusing it would be an
    // allowlist of one version pretending to be a compatibility statement.
    expect(codex.binding.version("codex-cli 999.0.0\n")).toBe("codex-cli 999.0.0");
    expect(codex.binding.version("codex-cli 1.0.0")).toBe("codex-cli 1.0.0");

    // Everything that is not exactly one canonical line is not an answer.
    expect(codex.binding.version("codex-cli")).toBe(undefined);
    expect(codex.binding.version("codex-cli v0.153.2")).toBe(undefined);
    expect(codex.binding.version("codex-cli 0.153")).toBe(undefined);
    expect(codex.binding.version("codex-cli 0.153.2-beta.1")).toBe(undefined);
    // One canonical line is still not one answer when something else came with
    // it: a wrapper that prints a banner has not told us which build ran.
    expect(codex.binding.version("warning: config\ncodex-cli 0.153.2")).toBe(undefined);
    expect(codex.binding.version("codex-cli 0.153.2\nsomething else")).toBe(undefined);
  });

  it("NL18c: the admitted string is the session's binding, and a later build that differs refuses", function* () {
    const codex = nativeAdapterFor("codex");
    if (codex === undefined || !bindsBuild(codex)) {
      throw new Error("codex must bind a build");
    }

    // Whatever canonical line admission accepted is what the session is bound
    // to — not the release this build was proven at.
    const accepted = codex.binding.version("codex-cli 999.0.0\n");
    expect(accepted).toBe("codex-cli 999.0.0");
    const prepared: ExecutableBuildBindingV1 = {
      schema: "executable-build.v1",
      reportedVersion: accepted!,
      executableDigest: { algorithm: "sha256", value: "a".repeat(64) },
    };

    // Reobserving the same build agrees.
    expect(sameExecutableBuild(prepared, { ...prepared })).toBe(true);
    // A later Codex — canonical, and admissible on its own — is still not the
    // build this session was prepared against. Admitting the product's shape
    // buys no continuation across releases.
    expect(
      sameExecutableBuild(prepared, { ...prepared, reportedVersion: "codex-cli 1000.0.0" }),
    ).toBe(false);
    // Same version, different executable: also not the same build.
    expect(
      sameExecutableBuild(prepared, {
        ...prepared,
