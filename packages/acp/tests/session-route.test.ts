...
describe("Tier SR — the construction route", () => {
  ...
  it("SR3: a client-native record carries no path, build, or instruction text", function* () {
  ...
  it("SR13: a publication whose write failed leaves neither staging file nor winner", function* () {
    // The namespace holds exactly the winners. A staging file is how a winner
    // is made durable before anything can observe it, and a write that failed
    // part-way is precisely when one would be left behind — so cleanup is
    // registered before the write rather than after it.
    const root = yield* workspace();
    const directory = path.join(root, "routes");
    const original = Deno.writeTextFile.bind(Deno);
    let failed = false;
    let store;
    ...
    try {
      Object.defineProperty(Deno, "writeTextFile", {
        configurable: true,
        // Writes a partial record, then fails: the state the contract cares
        // about is a file that exists and is not a winner.
        value: (target: string | URL, data: string, options?: Deno.WriteFileOptions) => {
          if (!failed && String(target).endsWith(".staging")) {
            failed = true;
            return original(target, data.slice(0, 12), options).then(() => {
      ...
      // Built while the fault is installed: the store reads its filesystem
      // surface off the host once, when it is created.
      store = createDenoSessionRouteStore(root)!;
      ...
      Object.defineProperty(Deno, "writeTextFile", { configurable: true, value: original });
    }

    // Nothing at all: no winner, and no residue beside where one would go.
    expect(yield* until(readdir(directory))).toEqual([]);
    expect(yield* store.read(KEY)).toBe(undefined);

    // And the namespace still works: the next publication creates the sole winner.
    const winner = yield* store.publish(clientNative());
    expect(yield* store.read(KEY)).toEqual(winner);
    expect(yield* until(readdir(directory))).toEqual([`${agentSessionKeyDigest(KEY)}.json`]);
  });
