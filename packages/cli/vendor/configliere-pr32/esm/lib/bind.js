...
function fromRead(param, read, rest) {
    ...
    let result = Array.isArray(value) ? decodeMany(param, value, path) : decode(param, value, typeof value === "string" ? param.decode(value) : [value], path);
...
function decodeMany(param, values, path) {
    let candidates = [[]];
