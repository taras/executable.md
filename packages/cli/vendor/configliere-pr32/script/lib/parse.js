...
exports.parse = parse;
const bind_js_1 = require("./bind.js");
const env_js_1 = require("./env.js");
const tokenize_js_1 = require("./tokenize.js");
const tokenizer_js_1 = require("./tokenizer.js");
const values_js_1 = require("./values.js");
function parse(route, input) {
    let tokenizer = new tokenizer_js_1.Tokenizer((0, tokenize_js_1.tokenize)(input.argv));
    ...
    let rest = {
        ...
        values: new values_js_1.Values().mount([], input.values ?? []),
        envs: new env_js_1.Envs().mount([], input.envs ?? []),
    };
    return resume({
        segments: [{
                id: "/",
...
function resume(state) {
    while (true) {
        ...
        let binding = (0, bind_js_1.bindPhase)({
            phase,
            segment: {
                range: {
                    start: segment.start,
...
function applyTransforms(segment) {
    let operations = segment.phases[0].transforms ?? [];
    let model = segment.model;
    for (let op of operations) {
        if (typeof op.transform === "function") {
            let result = op.transform(pick(model, op.keys), model, segment.phases[0].params);
            if (result !== undefined) {
                if (!record(result)) {
...
function invalidTransformResult() {
    return {
        ok: false,
