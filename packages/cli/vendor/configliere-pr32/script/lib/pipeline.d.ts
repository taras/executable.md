import type { AddField, AddParamToLast, AddRoutesToLast, AnyPhase, AnyPhases, AnyRoute, ChildrenOf, Done, Method, MethodsOf, ModelOf, ModelSchema, Next, Phase, Route } from "./types.js";
...
export interface ModelTransformElement<F, E extends readonly Unary[]> {
    readonly [operation]: AddModelTransform<F, E>;
    <const N extends string, const M extends Method, const T extends object, const C extends readonly AnyRoute[], const P extends AnyPhases>(route: Route<N, M, T, C, P>): Apply<Route<N, M, T, C, P>, AddModelTransform<F, E>>;
}
type ModelTransformOutput<F, T extends object> = F extends ModelSchema<infer Output> ? Output extends Record<string, unknown> ? Output : T : F extends (options: never, model: infer Input, phase: never) => infer Output ? T extends Input ? (Output extends Record<string, unknown> ? Output : T) : never : never;
...
type Delta = Identity<unknown> | AddMethod<Method> | AddParam<string, unknown> | AddRoutes<readonly AnyRoute[]> | AddModelTransform<unknown, readonly Unary[]> | Batch<readonly Unary[]> | Dynamic<unknown, AnyElement> | Custom<Transform>;
...
interface AddModelTransform<F, E extends readonly Unary[]> {
...
type InputOfDelta<D extends Delta> = D extends Identity<infer Input> ? Input : D extends AddMethod<Method> | AddParam<string, unknown> | AddRoutes<readonly AnyRoute[]> | AddModelTransform<unknown, readonly Unary[]> | Dynamic<unknown, AnyElement> ? AnyRoute : D extends Batch<infer E> ? InputOfPipeline<E> : D extends Custom<infer F> ? F["input"] : never;
type TakeStatic<E extends readonly AnyPipelineElement[], Fields extends readonly object[] = readonly [], Routes extends readonly AnyRoute[] = readonly [], Methods extends Method = never, Count extends readonly unknown[] = readonly []> = Count["length"] extends 20 ? readonly [MergeFields<Fields>, Routes, Methods, E] : E extends readonly [
    infer Head extends AnyElement,
    ...infer Tail extends readonly AnyPipelineElement[]
] ? DeltaOf<Head> extends Identity<unknown> ? TakeStatic<Tail, Fields, Routes, Methods, readonly [...Count, unknown]> : DeltaOf<Head> extends AddMethod<infer M> ? TakeStatic<Tail, Fields, Routes, Methods | M, readonly [...Count, unknown]> : DeltaOf<Head> extends AddParam<infer K, infer V> ? TakeStatic<Tail, readonly [...Fields, {
    [P in K]: V;
}], Routes, Methods, readonly [...Count, unknown]> : DeltaOf<Head> extends AddRoutes<infer C> ? TakeStatic<Tail, Fields, readonly [...Routes, ...C], Methods, readonly [...Count, unknown]> : readonly [MergeFields<Fields>, Routes, Methods, E] : readonly [MergeFields<Fields>, Routes, Methods, E];
...
] ? keyof Head | FieldKeys<Tail> : never;
type FieldValue<F extends readonly object[], K extends PropertyKey> = F extends readonly [
    ...infer Rest extends readonly object[],
    infer Last extends object
] ? K extends keyof Last ? Last[K] : FieldValue<Rest, K> : never;
...
type TransformModelInLast<P extends AnyPhases, Output extends object> = P extends readonly [AnyPhase] ? readonly [
...
type TransformModelPhase<P extends AnyPhase, Output extends object> = P extends Next<infer _Model, infer Routes, infer Requirement> ? Next<Output, Routes, Requirement> : P extends Done<infer _Model, infer Routes> ? Done<Output, Routes> : never;
...
type MaterializePhase<P extends AnyPhase> = P extends Next<infer Model, infer Routes, infer Requirement> ? Next<{
    [K in keyof Model]: Model[K];
}, Routes, Requirement> : P extends Done<infer Model, infer Routes> ? Done<{
    [K in keyof Model]: Model[K];
}, Routes> : never;
