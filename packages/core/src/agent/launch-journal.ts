 ...
 *
 * The digest is required and exact — it is what names the build this account
 * describes, and the account is only ever held to the other durable record of
 * the same observation, never to a build installed later. A present
...
export function parseMaterialized(value: unknown): MaterializedLaunchRecord | undefined {
  ...
  if (
    !declaredMembers(
      value,
      ["phase", "promptVersion", "requestId", "usage", "response"],
      ["nativeSessionId", "turn", "durationMs", "stopReason", "failure"],
    )
  ) {
    return undefined;
  }
