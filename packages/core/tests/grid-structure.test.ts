...
describe("Tier TG — the grid grammar", () => {
  it("TG1: refuses an unknown prop and `as` on the grid", function* () {
    const unknown = yield* runGrid('<Grid columns={2} layout="tiled"><Pane title="A" /></Grid>');
    expect(soleError(unknown)).toContain('<Grid> only accepts a "columns" prop. Got: "layout".');

    const captured = yield* runGrid('<Grid columns={2} as="grid"><Pane title="A" /></Grid>');
    expect(soleError(captured)).toContain('<Grid> only accepts a "columns" prop. Got: "as".');
    reachedNothing(unknown);
    reachedNothing(captured);
  });

  it("TG1: refuses an unknown prop and `as` on a pane", function* () {
    const unknown = yield* runGrid('<Grid columns={2}><Pane title="A" shell="zsh" /></Grid>');
    expect(soleError(unknown)).toContain('<Pane> only accepts a "title" prop. Got: "shell".');

    const captured = yield* runGrid('<Grid columns={2}><Pane title="A" as="pane" /></Grid>');
    expect(soleError(captured)).toContain('<Pane> only accepts a "title" prop. Got: "as".');
    reachedNothing(unknown);
    reachedNothing(captured);
  });

  it("TG1: requires columns to be a positive integer, however it was written", function* () {
    const missing = yield* runGrid('<Grid><Pane title="A" /></Grid>');
    expect(soleError(missing)).toContain('<Grid> requires a "columns" prop (a positive integer).');

    for (const literal of ["{0}", "{-1}", "{2.5}", '"2"', "{null}"]) {
      const run = yield* runGrid(`<Grid columns=${literal}><Pane title="A" /></Grid>`);
      expect(soleError(run)).toContain('Prop "columns" on <Grid> must be a positive');
      reachedNothing(run);
    }

    // The same rule reaches a value the document computes, which the source
    // could not have decided about.
    const computed = yield* runGrid('<Grid columns={size}><Pane title="A" /></Grid>', { size: 0 });
    expect(soleError(computed)).toContain(
      'Prop "columns" on <Grid> must be a positive integer. Got: 0.',
    );
    reachedNothing(computed);
  });

  it("TG1: requires a non-empty title on every pane, however it was written", function* () {
    const missing = yield* runGrid("<Grid columns={2}><Pane /></Grid>");
    expect(soleError(missing)).toContain(
      '<Pane> requires a "title" prop (the label the pane displays).',
    );

    for (const literal of ['""', "{3}", "{null}"]) {
      const run = yield* runGrid(`<Grid columns={2}><Pane title=${literal} /></Grid>`);
      expect(soleError(run)).toContain('Prop "title" on <Pane> must be a non-empty string');
      reachedNothing(run);
    }

    const computed = yield* runGrid("<Grid columns={2}><Pane title={label} /></Grid>", {
      label: "",
    });
    expect(soleError(computed)).toContain(
      'Prop "title" on <Pane> must be a non-empty string. Got: "".',
    );
    reachedNothing(computed);
  });

  it("TG1: refuses a self-closing grid", function* () {
    const run = yield* runGrid("<Grid columns={2} />");
    expect(soleError(run)).toContain("<Grid> holds the panes it lays out");
    reachedNothing(run);
  });
});
...
describe("Tier TG — structural placement", () => {
  it("TG2: refuses a grid with no pane", function* () {
    const run = yield* runGrid("<Grid columns={2}></Grid>");
    expect(soleError(run)).toContain("<Grid> requires at least one <Pane> pane.");
    reachedNothing(run);
  });

  it("TG2: refuses ordinary text written directly in a grid", function* () {
    const run = yield* runGrid('<Grid columns={2}>a note<Pane title="A" /></Grid>');
    expect(soleError(run)).toContain(
      '<Grid> holds only <Pane> panes. Found text "a note" directly inside it.',
    );
    reachedNothing(run);
  });

  it("TG2: refuses a direct element that is not a pane", function* () {
    const run = yield* runGrid('<Grid columns={2}><Boom /><Pane title="A" /></Grid>');
    expect(soleError(run)).toContain(
      "<Grid> holds only <Pane> panes. Found <Boom> directly inside it.",
    );
  ...
  it("TG2: refuses a control structure that would produce the panes", function* () {
    ...
    expect(messages[0]).toContain("<Grid> holds only <Pane> panes. Found <If> directly inside it.");
    expect(messages[0]).toContain("Write control flow inside a pane instead.");
    expect(messages[1]).toContain("<Pane> must be a direct child of <Grid>.");
  ...
  it("TG2: refuses a grid nested inside a pane", function* () {
    const run = yield* runGrid(
      [
        "<Grid columns={2}>",
        '<Pane title="Outer">',
        '<Grid columns={1}><Pane title="Inner" /></Grid>',
        "</Pane>",
        "</Grid>",
      ].join("\n"),
    );
    expect(soleError(run)).toContain("<Grid> cannot be written inside another <Grid>.");
    reachedNothing(run);
  });

  it("TG2: refuses a pane written outside every grid", function* () {
    const alone = yield* runGrid('<Pane title="A">Instructions.</Pane>');
    expect(soleError(alone)).toContain("<Pane> must be a direct child of <Grid>.");
    ...
    expect(soleError(buried)).toContain("<Pane> must be a direct child of <Grid>.");
