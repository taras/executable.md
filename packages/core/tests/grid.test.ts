  ...
  return registerComponents([
    {
      ...
      *fn() {
        ...
        if (pane === undefined) {
          throw new Error("<Interactive /> is written inside a <Pane> pane");
        }
    ...
    },
    {
      ...
      *fn() {
        const pane = yield* paneTerminal();
        if (pane === undefined) {
          throw new Error("<Slow /> is written inside a <Pane> pane");
        }
        yield* pane.interactive(function* (spawned) {
          yield* sleep(25);
...
): Operation<GridAuthority> {
  return (function* (): Operation<GridAuthority> {
...
function plainDocument(columns: number, panes: string[]): string {
  return [`<Grid columns={${columns}}>`, ...panes, "</Grid>", ""].join("\n");
}
...
const PANES = ['<Pane title="Left">left<Interactive /></Pane>', '<Pane title="Right" />'];

describe("Tier TG — the terminal authority", () => {
  const GRID = ["<Grid columns={2}>", ...PANES, "</Grid>", ""].join("\n");
...
describe("Tier TG — a grid written in a document", () => {
  ...
  it("TG4: duplicate titles stay valid, and identity is the ordinal", function* () {
    ...
    const run = yield* runDocument(
      ...
      [
        "<Grid columns={2}>",
        '<Pane title="Agent">first<Interactive /></Pane>',
        '<Pane title="Agent" />',
        '<Pane title="Agent">third<Interactive /></Pane>',
        "</Grid>",
        "",
      ].join("\n"),
    );
  ...
  it("TG7: root output is flushed before the grid, and pane text stays in its pane", function* () {
    ...
    const run = yield* runDocument(
      ...
      [
        ...
        "",
        "<Grid columns={2}>",
        '<Pane title="Left">left text<Interactive /></Pane>',
        '<Pane title="Right">right text<Interactive /></Pane>',
        "</Grid>",
        "",
        "after",
        "",
  ...
  it("TG6: a paired pane runs every component in its body, in order", function* () {
    ...
    const run = yield* runInterrupted(
      dir,
      heldDocument(2, [
        '<Pane title="Two components"><Interactive /><Ran mark="second component" /></Pane>',
        '<Pane title="Shell" />',
      ]),
      stream,
...
describe("Tier TG — startup, settlement and teardown", () => {
  const TWO = ["<Grid columns={2}>", ...PANES, "</Grid>", ""].join("\n");
  ...
  it("TG9: a pane that never starts fails the grid, and nothing attaches", function* () {
    ...
    const run = yield* runDocument(
      ...
      [
        "<Grid columns={2}>",
        '<Pane title="Quiet">nothing interactive here</Pane>',
        '<Pane title="Shell" />',
        "</Grid>",
        "",
      ].join("\n"),
  ...
  it("TG9: an immediate spawn-and-exit is both ready and settled", function* () {
    ...
    const run = yield* runDocument(
      dir,
      ["<Grid columns={1}>", '<Pane title="Shell" />', "</Grid>", ""].join("\n"),
      {
        composite: {
  ...
  it("TG9: simultaneous startup failures report the first authored ordinal", function* () {
    ...
    const run = yield* runDocument(
      ...
      [
        "<Grid columns={2}>",
        '<Pane title="First">no interactive child</Pane>',
        '<Pane title="Second">no interactive child either</Pane>',
        "</Grid>",
        "",
      ].join("\n"),
    );
  ...
  it("TG12: close cancels a live pane as closed, then destroys and continues", function* () {
    ...
    const run = yield* runDocument(
      ...
      [
        "<Grid columns={1}>",
        '<Pane title="Live"><Interactive /><Hold /></Pane>',
        "</Grid>",
        "",
...
describe("Tier TG — durability and replay", () => {
  ...
  it("TG16: a completed pane is restored; an incomplete shell starts again", function* () {
    ...
    const source = heldDocument(2, [
      '<Pane title="Left"><Ran mark="left ran" /><Interactive /></Pane>',
      '<Pane title="Right" />',
    ]);
  ...
  const PROP_BORNE = [
    ...
    "<Pane title={props.label}>left<Interactive /></Pane>",
    '<Pane title="Right" />',
    "</Grid>",
    "",
    `<Ran mark="${PAST_THE_GRID}" />`,
  ...
  it("TG17: a continuation opens the retained structure, not the file's", function* () {
    const structural: [string, string[]][] = [
      ["pane count", [...PANES, '<Pane title="Extra" />']],
      ["pane order", ['<Pane title="Right" />', ...PANES.slice(0, 1)]],
      ["pane form", ['<Pane title="Left" />', '<Pane title="Right" />']],
    ];
  ...
  it("TG19: a cancellation during reader-close teardown waits for it, and replays", function* () {
    ...
    const stream = new InMemoryStream();
    const source = heldDocument(2, [
      '<Pane title="Live"><Interactive /><Ran mark="pane body" /><SlowTeardown /></Pane>',
      '<Pane title="Shell" />',
    ]);
