...
import { registerStagingOwner, revokeStagingOwner, stagingFactory } from "./staging.ts";
import type { DurableStageFactory } from "./staging.ts";
...
export interface DurableRunOptions {
  /** Receive this root's staging capability before workflow work begins. */
  staging?: (factory: DurableStageFactory) => void;
...
export function* durableRun<T extends WorkflowValue>(
  ...
  registerStagingOwner(ctx);

  try {
    options.staging?.(stagingFactory(ctx));
