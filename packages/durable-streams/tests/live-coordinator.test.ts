...
describe("Tier DLC — live durable-operation coordination", () => {
  ...
  it("DLC8: an explicit coordinator affects only its selected operation", function* () {
    const stream = new InMemoryStream();
    const identity = establishJournalProvenance(stream);
    let coordinated = 0;
    let ordinary = 0;
    const coordinator: LiveDurableOperationCoordinator = {
      *run<T extends Json>(
        ...
        expect(journalProvenance).toBe(identity);
        expect(Reflect.get(journalProvenance ?? {}, "append")).toBe(undefined);
        expect(Reflect.get(journalProvenance ?? {}, "readAll")).toBe(undefined);
        coordinated += 1;
        const result: Result = { status: "ok", value: yield* execute() };
        yield* publish(result);
        return result;
