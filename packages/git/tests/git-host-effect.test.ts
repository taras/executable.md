...
describe("Tier GH — shared external Git-host effect reconciliation", () => {
  ...
  it("GH17: erasing the shared identities cannot buy a live Git-host effect", function* () {
    ...
    // holds one for.
    //
    // The name is the one the Git-host journal admission installs. #822 gave
    // each admission its own stable name and its own fresh value; what this
    // case is about — a holder of the name erasing what it holds — is
        ...
        yield* registerComponents([
          {
            ...
            props: { type: "object", properties: {}, additionalProperties: false },
            *fn() {
              // The queue is bound by name, so a document could rebind it. The run
              // itself is untouched; only what sits beside it moves.
              const slot = createContext<readonly unknown[] | undefined>(
                "executablemd.git.git-host.retained-identities",
                undefined,
              );
              const genuine = yield* slot.get();
              expect(genuine).toBeDefined();
              yield* slot.set(substitute);
    ...
    yield* registerComponents([
      {
        ...
        *fn() {
          if (options.erase === true) {
            ...
            const genuine = yield* slot.get();
            // The name really was bound, so what follows erases something.
            expect(genuine).toBeDefined();
            yield* slot.set([]);
            yield* Divergence.around({ decide: () => ({ type: "run-live" }) });
          }
