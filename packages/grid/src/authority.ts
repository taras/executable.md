...
export interface GridAuthority {
  present(request: GridRequest, composite: GridComposite): Operation<void>;
}
...
export interface LiveGrid {
  ...
  run(composite: GridComposite): Operation<void>;
...
const Installation: Context<GridInstallation | undefined> = createContext<
 ...
 */
export function* useGridInstallation(): Operation<GridAuthority> {
...
export function gridInstallation(): Operation<GridInstallation | undefined> {
 ...
 */
export function createGridClaims(request: GridRequest): GridClaims {
  validate(request);

  let sealed = false;
  const claims: PaneClaim[] = [];
...
export function sealOnTeardown(claims: GridClaims): Operation<void> {
  return ensure(() => {
    claims.seal();
  });
}
