 ...
 * it can be recognized later, and the provider will not act on one without an
 * observer to ask. There is no real executable here, so this answers the same
 * question deterministically: one stable digest and one canonical version for
 ...
 * watch a build drift replaces what this answers.
 */
