 ...
 *
 * The journey is `packages/test-agent/src/GridNativeLaunch.test.md`,
 ...
 * what it was asked to start, and the grid provider, which presents
 * nothing.
 *
...
const SCENARIOS = [
  "GridNativeLaunch.planner.md",
  "GridNativeLaunch.implementor.md",
  "GridNativeLaunch.reviewer.md",
];
...
interface Run {
  ...
  request?: GridRequest;
  /** Whether a grid provider was asked for a grid at all. */
  grids: number;
...
interface RunOptions {
  ...
  /** Install a grid provider; omit for a host that cannot present one. */
  provider?: false;
  ...
  onState?: (ordinal: number, state: PaneState) => void;
...
function* runJourney(options: RunOptions = {}): Operation<Run> {
  ...
  let grids = 0;
  let request: GridRequest | undefined;
              ...
              *open([asked]) {
                ...
                const composite = yield* prepareControlledComposite(asked, {
                  ...
                  *onDestroy() {
                  ...
                  onUpdate(ordinal: number, state: PaneState) {
...
const ONE_SESSION = [
  "<TestAgent>",
  '<TestAgent.Scenario session="planner" src="./GridNativeLaunch.planner.md" />',
  "",
...
const FAILING_AND_SURVIVING = [
  "<TestAgent>",
  '<TestAgent.Scenario session="planner" src="./GridNativeLaunch.planner.md" />',
  '<TestAgent.Scenario session="implementor" src="./GridNativeLaunch.implementor.md" />',
  "",
...
describe(
  ...
  () => {
    ...
    it("GN5: with no grid provider, a pane launch starts nothing at all", function* () {
