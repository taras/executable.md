...
import { registerTerminalProvider, reserveTerminal, TerminalGrids } from "@executablemd/terminal";
import {
...
describe("Tier TG22 — mixed native providers", () => {
  ...
  it("TG22: materialization, native readiness and reader-close cleanup stay independent", function* () {
    ...
    yield* registerTerminalProvider("mixed-controlled", function* (_settings, authority) {
      yield* TerminalGrids.around(
        {
          *open([request]) {
    ...
    yield* installTerminalGridProfile({ provider: "mixed-controlled" });
    yield* installAgentComponents({
      rootProvider: {
