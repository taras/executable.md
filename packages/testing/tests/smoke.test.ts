...
const EMBEDDED_TESTS = [
  ...
  "Let binds inline content",
  "Let select extracts the matching node",
  "Let binds a direct value rather than rendering it",
  "Let binds a preceding value by reference",
  ...
  "A Let binding from the selected branch stays available afterward",
...
describe("smoke test", { sanitizeOps: false, sanitizeResources: false }, () => {
  ...
  it("smoke document passes its embedded tests live and on replay", function* () {
    ...
    const options: ExecuteOptions = {
      ...
      stream,
      includes: ["smoke-test", "packages/core/components"],
    };
