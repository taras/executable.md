/**
 * Tier WF — a `<Git.Commit>` across a process boundary.
 *
 * At the kill point the commit transaction is open with the imported object, the
 * moved branch, the published Workspace root and the routed journal row all
 * written; `SIGKILL` then runs no application cleanup, so nothing commits and
 * nothing rolls back. What a later connection finds is decided there.
 *
 * The write and the staging that produced the index each commit in their own
 * effect before the commit begins, which is what makes the observation sharp:
 * the recovered database must hold a staged file and a branch that never moved.
 *
...
import { CRASH_MESSAGE, CRASH_PATH, MAIN_CONTENT } from "./support/git-crash-process.ts";
...
describe("workflow Git.Commit across a process boundary", () => {
  it("commits the whole transition or none of it when a real SIGKILL lands", function* () {
    const root = yield* useStorageRoot();
    const remote = yield* useBareRemote(REMOTE);
    const runId = "git-commit-crash";
    ...
    // Inside the crashed transaction the commit's journal row and its published
    // root both existed.
    ...
    // Repository, the file write and the staging all committed before it began.
    expect(during.outside["types"]).not.toContain(WORKSPACE_GIT_COMMIT);
    ...
    // the staged file and a branch that never moved.
    const inspected = announced(yield* runChild(["inspect", root, runId]));
    expect(inspected["repositories"]).toBe(1);
    expect(inspected["staged"]).toEqual([CRASH_PATH]);
    expect(inspected["headMessage"]).toBe("first\n");
    expect(inspected["types"]).not.toContain(WORKSPACE_GIT_COMMIT);
    expect(inspected["currentRoot"]).toBe(during.outside["currentRoot"]);
    expect(CRASH_MESSAGE).toBe("recorded by the crash child");
  });
});
