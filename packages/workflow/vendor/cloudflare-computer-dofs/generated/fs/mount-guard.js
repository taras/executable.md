...
export function invalidateReadOnlyMountCache(db) {
    cache.delete(db);
}
function loadReadOnlyRoots(db) {
    const rows = db.all("SELECT root FROM _vfs_mounts WHERE mode = 'read-only'");
    const roots = rows.map((r) => r.root);
    cache.set(db, roots);
    return roots;
}
export function getReadOnlyMountRoots(db) {
...
export function assertNotReadOnly(db, path) {
...
export function readOnlyRootFor(db, path) {
    const roots = getReadOnlyMountRoots(db);
    for (const root of roots) {
        if (overlapsRoot(path, root))
            return root;
