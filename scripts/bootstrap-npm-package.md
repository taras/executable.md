...
> [!WARNING]
> Run this **without `-j`/`--journal` and without `--verbose`**. The one-time
> password you enter is interpolated into the publish and trust commands, and
> those commands are what a durable entry records and what `--verbose` reports
> to stderr. Rendered output carries only what a command printed, so the code
> stays out of the document — but a journal file or a verbose stderr stream
> would hold it. Neither is needed here.
>
> `specs/release-process-spec.md` §6 carries the command to run this with.
