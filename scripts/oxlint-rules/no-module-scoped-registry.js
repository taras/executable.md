 ...
 * that made it ends.
 *
 *     export function* useSegmentCauses() {
 *       const causes = new WeakMap();
 *       yield* SegmentCauses.set(causes);
 *       return causes;
 *     }
 *
 * Moving run state onto the objects it describes — a symbol brand on each one —
 * is not the answer: it is the same lifetime, spread across more places, and a
 * `Symbol.for` key is itself a process-global anyone can forge.
 *
 ...
 *
 * One thing is not run state at all: metadata an author declares about a
 * definition at module evaluation, outside any operation — `printErrors(fn)`
 * marking a component function. There is no run to own it and no scope to reach,
 * so it belongs on the definition itself, written under a module-private
 * `Symbol()`. That is a declaration about one value, not a table this module
 * fills, which is why no table appears here for this rule to report.
 *
 * What this rule cannot see is accumulation inside a library object — an `Ajv`
 * instance caching every schema it ever compiled, in a `Map` of its own. The
 * architecture rule covers it ("not hidden inside library objects that
 * accumulate"); a reviewer holds that half.
 */
