...
const DENO_ONLY_TOOLING: RuntimeExclusion[] = [
  ...
  {
    path: "scripts/tests/repl-components.test.ts",
    reason:
      "its subject is the component boundary of the same Deno terminal harness: it mounts a Freedom tree, renders through `@bomb.sh/tty` and reads the frame back as cells. A Node or Bun shard has no equivalent of the host it is testing",
  ...
  {
    path: "packages/acp/tests/loaded-copy.test.ts",
    reason:
      "builds the second copy of core's placement machinery with `deno bundle`, which is Deno's, and drives the real ACP provider against it; the classification it proves — a resolved Session is recognized by its public data and everything else is handed to the delivered coordinator — is runtime-neutral and the same dispatch is exercised on every runtime by packages/acp/tests/provider.test.ts",
    issue: DERIVED_SCOPE,
  },
  {
  ...
  {
    path: "packages/cli/tests/pull-request-read-fork.test.ts",
    reason:
      "forks a real run through the compiled CLI against a loopback GitHub, reading the node:sqlite run store the Deno host wrote; the workflow commands exist on the Deno entrypoints alone and Bun has no node:sqlite at all",
