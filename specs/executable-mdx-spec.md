...
| DLC13 | Runtime-neutral boundary | No module of the shared coordination surface names a runtime, storage, connection, savepoint, transaction-token or process-global type outside its own prose, and none imports a specifier only one host can resolve — `node:`, `bun:`, `deno:` and `cloudflare:` modules, an adapter, a vendored source or a host process — recognized by shape rather than by an enumerated list |
