...
The root document receives its declared inputs from programmatic `props` or
from the sources generated by `xmd run`. Source names, precedence, help, and
failure behavior are defined in
[Root Document Inputs](./root-document-inputs-spec.md).
...
its input schema without executing the document or creating a journal. Root
