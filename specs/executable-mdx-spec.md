...
#### 5.3.1 `<Syntax />`, the protected symbols component
...
symbol construction and one Markdown renderer serve both, so an operator
...
renders the symbols available where it is written. It is a **text component**: the ordinary
...
component for are each refused before the symbols are read, so a refusal
produces no partial text and no successful retained result.

**The named form renders documentation.** `<Syntax names={["Elicit", "File"]} />`
renders each selected component's symbol metadata followed by the long-form
documentation its owning package ships, once each, **in symbol order** whatever
...
selects from the **enclosing authoring symbols** and states, per entry,
`**Available in this evaluation:** yes` or `no`. At a root the two are one set
of symbols and everything selected is available; a trusted evaluation boundary that
...
prose rather than a symbol entry, and putting it in routine output would make
...
**The symbols are the execution's own.** Canonical core builds them at the root from
...
**The symbols say where a component came from.** A protected component is reported under
the symbol origin kind `protected`, carrying the canonical core origin — never
...
additions, so the symbols are **version 2**: a version-1 reader was promised a
...
and a form no invocation has are each refused there, so two entries can be
...
**A partial set of symbols is never presented as a complete one.** A missing include
...
the same symbols as JSON with `--json`, both from one inspection. Its only
...
**The command has not succeeded until stdout has taken every byte.** A
pipe holds far less than the whole rendering, so backpressure changes how long the command
...
A selector must resolve to exactly one symbol entry. Zero matches and several
...
a catalog, a match list, or a kind that the recorded document contradicts is
...
targets.` when the catalog are empty. Every other failure keeps the printed-error
...
the symbols advertise and what a refusal says all come from one value. Any paired
...
installer, its own upgrade assembly and its own repository installer to
`runXmd(args, installService, upgrade, installRepositories, ...)`:
...
fact: what this runtime can *operate*, rather than what it is. Deno and the
compiled binary pass the live provider — `denoRunRepositories(HELPER)`, built
from the same credential-helper assembly the workflow host uses — and Node and
Bun pass `unsupportedRepositories`. Every runtime registers the same thirteen
declarations either way, so `xmd syntax` describes one language everywhere and a
runtime that operates none of it reports an absent provider before a lock, a
credential, a subprocess or a request exists (§5.3, §8.1).
...
- a workflow execution installs none at all, because its Workspace attachment
  owns repositories for the run.

The service installer is invoked only for `xmd run` and `xmd test`, immediately
before `execute()`. Help, inspection and agent-worker paths never install or
attach a service. Document-aware `xmd run … --help` is one of those inspection
paths and never invokes the installer.
...
recovery boundaries themselves are Tier OM's and Tier RF's, the symbols row is
...
| SY13b | An unreadable reachable directory | An include root that refuses, and a valid-name directory beneath it that refuses, each fail the whole request rather than shortening the symbols |
...
| SY19–SY22 | Markdown documentation | String `description`/`as`/`context` reach the symbols; a non-string value documents nothing; an undocumented component stays complete; a declared `returns` reports `value` mode with its schema |
...
Named `SYN` rather than `SY` or `SL`, which already name the syntax symbols and
own-scope context updates. The symbols *value* is Tier SY's; this tier is the
...
| SYN29 | The named form | Selected entries render their metadata and documentation once each in symbol order, not request order, with nothing else of the symbols; `as` binds identical text |
...
| SYN46 | Cancelling a named read | Teardown completes and no successful `syntax_symbols` record is committed |
...
| FE5b | One name, one implementation | A provider-backed name held by two entries across disjoint forms and both tables is resolved by one lookup and sealed as one shared definition and schema; a second identity for that name — within one table, across the two, or under another origin — refuses at capture naming both spellings rather than letting assembly order decide; and one name held as both a capability and a component answer refuses, because core supplying the body and the chain answering for it are different grants |
...
| FE18 | The durable protocol is closed | A version-1 admission still resumes under its own ceilings and a version-1 refusal replays as the refusal it recorded, both from literal records an older run actually wrote: the opaque string is reconciled against a current capability identity under the `origin#key@revision` spelling that build used. A string spelling another identity refuses, and a string never describes a component answer, because that arm did not exist when version-1 records were written. A version-2 identity written as a string, with no kind, with an unknown kind, with an extra member, with a non-string member, as an array or as null each refuse; so do a tagged result holding an untagged policy, a mixed or unknown version, an extra or missing member, a duplicate or empty class or form list, and a malformed request field. Each version has its own positive control that resumes on the unmodified record, so no row passes because a forged record never resumes at all. Frozen as `FE18/GX21z`, `FE18/GX21x` and the `FE18/GX21y` table in `generated-xmd.test.ts` |
| FE14 | The chain answers, and the answer is held to its identity | A middleware-only name resolves to the provider's implementation and a fragment runs it; a fresh admission retains the exact structural identity. Two providers genuinely compose: the inner one claims A and the outer one delegates to it — proven by the object delegation returned — and then answers with its own claimed B, so B is what runs and A does not; the same composition with an *unclaimed* outer answer refuses before either body. An unidentified, a copied and a post-claim-mutated answer each refuse before the root import; an answer under another identity refuses. A continuation supplying both live implementations — the unchanged A behind the inner provider and a changed B behind the outer — runs neither when the retained identity moved, while an unchanged continuation resumes and invokes once. An answer whose props schema reads differently on every read is sealed as the claim-time schema, so the substitution never validates a fragment. Two occurrences share one capture lookup and a provider that would answer differently is never asked again; a capability-only profile resolves no name at all; and a provider still answering when a fragment resolves is refused by the witness |
| FE15 | A provider's answer belongs to the execution that captured it | Teardown happens on all three endings: a resolution cancelled while suspended waits for its cleanup and publishes no profile, root import, admission or body; a successful run and a failed one each leave a claimant that refuses afterwards. A *losing* claimant used from a running document — while the winning execution is still live — records its own statement, cannot retag the winner's implementation, and changes nothing about what the sealed profile runs. Two executions live at once each run the answer they captured, with the first suspended inside its admitted body while the second completes. A captured body refuses after teardown; a later execution without the provider finds nothing the first left; and cancelling an admitted body waits for its cleanup and records no terminal. The unit half is `answer-identity.test.ts`, which adds that identification answers with the claim-time copy and that an alternating answer cannot launder one through a second read; the execution half is `evaluate-provider-lifetime.test.ts` |
| FE30 | A layered bootstrap grants nothing | One package bootstrap — documentation, registrations *and* the component-answer provider that answers for the admitted name — is entered through an inherited layer and again through an execution-local one. Both registrations resolve, the ordinary component runs, the named documentation renders once, both provider layers are in the chain with the outer one observing and delegating, and the locally claimed identity is the one the capture admitted and the fragment ran. Teardown belongs to the execution: the local bootstrap's claimant states nothing once the child run is over. The bootstrap's own registered, documented name is still refused inside a fragment; a provider claiming a revision the layered host did not admit refuses at capture, before the root import; and a non-identical overlap on one owner and component refuses at the child collection boundary in either order, with the provider never asked. Receives #765's `SYN25l.5` and its overlap refusal rather than cloning the collector |
...
### Tier DT — Document target symbols, selectors, and projection

| # | Test | Verify |
|---|------|--------|
| DT1–DT5 | Outline | ATX and Setext headings symbols in source order; a skipped depth still nests; the outermost depth is the smallest present; a sole outermost heading is the title and several are path levels |
...
| DT18/DT19 | Empty symbols | A document with no heading addresses nothing, and a sole title is no target |
| DT20–DT22 | Matching | Literal levels, embedded `*`, and `**` across zero or more levels |
| DT23 | Exactly one | Zero matches and several matches both fail, reporting matches and the symbols |
...
| DT44–DT47 | Inspection | The symbols report without selecting; a glob resolves to the exact target; an unresolvable target fails inspection; the failure's data is frozen and rebuilt |
...
| DT79/DT80 | Structured symbols | `targetInfo` carries the same targets, order and duplicates as `targets`, each with its own description, on the unselected path and beside an unchanged exact `target` on the selected one |
...
The shared entrypoints carry the rest: `syntax-symbols.test.ts` SY4b and SY5b
freeze both symbol entries and prove no repository file or registration can
...
| GX14b–GX14c | One ceiling per identity | Two admitted identities whose four terms differ only in where a space falls each perform exactly their own admitted request, and neither entry's limit admits the other's — so the run's ceiling table is keyed by the terms rather than by a joined spelling |
...
row: it inspects the symbols without running anything.
...
| # | Test | Verify |
|---|------|--------|
| ORC1 | One declaration surface | All thirteen names appear in the symbols with complete contracts; a repository file of one of those names shadows the default; symbol construction performs no ambient discovery, lock, Git, credential or network operation |
