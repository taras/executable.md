...
| GXC10 | The form at the invocation | A `Component.hasContent` handler outside the generated expansion that answers consistently — reporting content for a self-closing admitted read, or denying it for a paired admitted write — refuses that invocation before any provider is reached: zero reads, zero writes, the file unchanged, and the retained identity and form unmoved. A handler that observes and delegates leaves both forms running through their ordinary providers. A handler that answers per call is not covered, and cannot be from here |
