...
| WRH17 | Waking a machine wait | An authenticated intake for a merged-observation wait's exact subject is retained without an executor acquisition, appends no outcome and moves no stage; a later executor reobserves and only an adoption advances the run |
...
| WGE2b | Comment correlation | A provider that cannot write, preserve and completely query a stable opaque marker refuses before its first mutation; the authored logical body is sent byte for byte and the correlation representation rides outside it; the binding and every replay expose the body and the provider comment identity, never the encoding |
...
| WFP9e | Merged-observation wait | The wait record names the canonical pull-request URL, expected merge commit, terminal decision, revision and suspension identity, carries `retriesExhausted: true`, and holds no stage, outcome or verdict; consuming it authorizes one reobservation |
...
| WFL6c | Waiting for the host | A pull request still open runs the bounded host-configured retry and then suspends on the merged-observation wait; the wait appends no outcome and moves no stage, an intake or an explicit resume only permits reobservation, and terminal settlement stays absent until the adoption succeeds. A conflict observed on any attempt ends the wait as a conflict |
