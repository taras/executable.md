...
| WGAC7 | The import boundary | `importComponent` middleware calling the implementation with an invocation it built, with the first site's routed at the second, and with a live content-bearing parent's routed into the sites inside it, are each refused and admit nothing under a name that is not the site's own; delegating the genuine one admits each site under its own, nested or not; and an interrupted run resumes into each site's own record — the retained observation restored without re-reading, the interrupted request performed |
