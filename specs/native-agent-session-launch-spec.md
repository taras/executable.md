...
- a provider may replace the layer in place only when it can preserve the same
  ...
  invoking a model;
- a provider with an immutable layer may discard and recreate only an unused
  XMD-owned session shell that it can prove has never accepted a prompt and has
  never been handed to a native UI; and
...
the session's serialization lease until the native child exits.
...
A completed replay restores the terminal outcome and never launches a native
UI. A partial replay before launch reuses the recorded native session and
continues at the first incomplete phase. A partial replay whose native process
...
and history. Discard and recreation is limited to a provably unused XMD-owned
session shell. A cached transcript observed after a native handoff cannot prove
that condition.
...
15. concurrent launches cannot share one logical session or foreground
    terminal; and
16. instruction reconciliation resumes an equal layer, recreates only a
    provably unused XMD-owned shell, and refuses a changed layer after either a
    prompt or a native handoff even when the cached transcript is empty.
...
12. A changed instruction layer never discards retained or unobserved native
    history: it is replaced in place with identity and history preserved,
    applied to a provably unused XMD-owned shell, or refused before handoff.
