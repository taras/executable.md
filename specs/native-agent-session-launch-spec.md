...
6. `Session.Launch` takes the run's foreground-terminal lease. A host with no
   foreground terminal refuses here — before an agent is resolved, so learning
...
13. The provider spawns the native UI as an interactive child with the root
    terminal inherited — resuming, for a provider-returned adapter,
...
16. When the child exits, XMD records its terminal outcome, removes the private
    ...
    foreground-terminal lease. The route and the retained phases stay: they are
...
18. Execution continues after `Session.Launch` in its enclosing document flow.
...
V1 holds the foreground-terminal lease for the CLI execution. Two launches
cannot concurrently own that terminal, even when they name different sessions.
Sequential launches are ordinary composition.

Cancellation interrupts the native interactive process, establishes that it can
no longer execute or hold the foreground terminal, restores that terminal's
...
an inherited foreground-terminal interactive child with cancellation and
bounded reaping;
...
foreground sessions, automatic nested `AGENTS.md`
