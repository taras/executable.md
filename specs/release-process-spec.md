...
run against the source they review rather than a published binary.
...
- **`review.yml`** (`pull_request`): checks out the pull request, installs the
  pinned Deno version, runs `deno task deps`, builds `./dist/xmd`, and executes
  `.reviews/ReviewPR.md` with that checked-out binary. The root uses `<Output>`
  so execution errors make `xmd run` fail; review findings remain report text.
  The journal is uploaded with `if: always()` and no postflight parser inspects
  journal records or rendered error markers.
- **`repo-analysis.yml`** (`workflow_dispatch`): checks out the requested ref,
  prepares and builds that ref's binary, and executes
  `.reviews/AnalyzeRepoCI.md` with the same `<Output>` failure contract. Its
  report, journal, and run metadata are uploaded unconditionally.
